...
| 種類 | 1.13 |
|---|---|
| インストーラー | `ShareWorkin_v1.13_install.exe` |
| 配布 ZIP | `ShareWorkin_v1.13_Setup.zip` |
| ハッシュ一覧（ZIP 同梱しない） | `ShareWorkin_v1.13_SHA256.txt` |
...
Inno Setup 6 と .NET 8 SDK が入っている Windows 環境で、リポジトリ直下から実行します。
...
実装判断は `_works/0428-1生成レポート.md` と `_works/ShareWorkin_仕様書_v2_1.md` を参照します。
1.13 はフォルダー単位の共有状態を「全員 / 読みのみ / 共有OFF」で確実に揃える開発版として扱います。
